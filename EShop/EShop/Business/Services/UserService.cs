...
namespace EShop.Business.Services
{
    public class UserService : IUserService
    {
        ...
        public async Task<int> DestroyAllCustomerData(ApplicationUser user)
        {
            try
            {
                ...
                if (shoppingCart != null)
                {
                    ...
                    foreach (var scp in shoppingCartProducts) _context.Remove(scp);
                ...
                foreach (var scph in scphs) _context.Remove(scph);
        ...
        public async Task<Tuple<int, IQueryable<UserInRoleViewModel>>> QueryUsersInRolesByPageAsync(int pageNumber,
            int usersPerPage)
        {
            ...
            var totalUserCount = await (from u in _context.Users
                join ur in _context.UserRoles on u.Id equals ur.UserId
                join r in _context.Roles on ur.RoleId equals r.Id
                select new UserInRoleViewModel
                {
            ...
            var pageCount = totalUserCount / usersPerPage;

            if (totalUserCount % usersPerPage != 0) pageCount++;
