...
namespace EShop.Util
{
    ...
    {
        public static async Task<string> UploadImageAsync(this IHostingEnvironment appEnvironment, IFormFile file, string dirName = null, int uploadMaxByteSize = -1)
        {
            string uploads;
        ...
        public static async Task DeleteImageAsync(this IHostingEnvironment appEnvironment, string fileName, string dirName = null)
        {
            string uploads;
            if (dirName == null) uploads = Path.Combine(appEnvironment.WebRootPath, "images");
            else uploads = Path.Combine(appEnvironment.WebRootPath, "images\\" + dirName);
